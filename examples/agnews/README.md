...
End-to-end mlprobe demo on a real NLP dataset
...
Each runs through the standard mlprobe pipeline: declarative audit →
...
- **Probes run as local subprocesses.** Wall-clock is measured on *your*
  ...
  ([issue #1](https://github.com/dnewcome/mlprobe/issues/1)) or a
  SageMaker launcher.
- **The instance-type catalog uses approximate `us-east-1` on-demand
  ...
  pricing is
  [issue #2](https://github.com/dnewcome/mlprobe/issues/2).
